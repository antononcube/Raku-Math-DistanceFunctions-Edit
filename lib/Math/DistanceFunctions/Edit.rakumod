...
our proto sub edit-distance($s1, $s2 --> Int:D) is export {*}

multi sub edit-distance(Str:D $s1, Str:D $s2 --> Int:D) is export {
...
multi sub edit-distance(@s1, @s2 --> Int:D) is export {
    return do if (@s1.all ~~ Int:D) && (@s2.all ~~ Int:D) {
    ...
    } elsif (@s1.all ~~ Bool:D) && (@s2.all ~~ Bool:D) {
        return edit-distance(@s1».Int, @s2».Int,);
    } elsif (@s1.all ~~ Str:D) && (@s2.all ~~ Str:D) {
        my @words = [|@s1, |@s2].unique;
        my %reMap = @words Z=> (^@words.elems);
        return edit-distance(@s1.map({ %reMap{$_} }), @s2.map({ %reMap{$_} }));
    } else {
        return edit-distance(@s1».raku, @s2».raku);
    }
}